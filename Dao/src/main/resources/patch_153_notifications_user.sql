...
(
  ...
  CONSTRAINT `fk_2FA_USER_NOTIFICATION_MESSAGE_SETTINGS_user_id` FOREIGN KEY (`user_id`) REFERENCES `USER` (`id`) ON UPDATE CASCADE ON DELETE CASCADE,
  CONSTRAINT `fk_2FA_USER_NOTIFICATION_MESSAGE_SETTINGS_notificator_id` FOREIGN KEY (`notificator_id`) REFERENCES `2FA_NOTIFICATOR` (`id`) ON UPDATE CASCADE ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
