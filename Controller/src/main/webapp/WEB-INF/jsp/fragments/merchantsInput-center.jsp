                ...
                <div class="form-horizontal refill__money">
                    <div class="input-block-wrapper clearfix">
                        <div class="col-md-4 input-block-wrapper__label-wrapper" style="width:225px; padding-left: 0">
                            <label style="font-size: 15px" for="currencyName"
                    ...
                    <c:if test="${isAmountInputNeeded}">
                        <div class="input-block-wrapper clearfix">
                            <div class="col-md-4 input-block-wrapper__label-wrapper"
                                 style="width:225px; padding-left: 0">
                                            ...
                                            <c:if test="${(merchantCurrency.minSum > 0) && (merchantCurrency.processType != \"INVOICE\")}">
                                                <c:choose>
                                                    <c:when test="${merchantCurrency.processType == \"CRYPTO\"}">
                                                        <span><loc:message
                                                                code="merchants.input.recommendedMinSum"/></span>
                                                    </c:when>
                                                    <c:otherwise>
                                    ...
                                    <c:choose>
                                        ...
                                        <c:otherwise>
                                            <div style="overflow:auto; ">
                                                <div>
                                                    <div class="alert alert-warning"><loc:message
                                                            code="message.additional.deposit.warning"
                                                            arguments="${currency.name}"/></div>
                                                    <c:choose>

                                                        <c:when test="${merchantCurrency.additionalTagForRefillIsUsed}">
                                                            <div class="alert alert-success">
                                                                <loc:message
                                                                        code="refill.messageAboutCurrentAddressSimple"/>
                                                                <div id="address-to-pay">
                                                                    <p class="pay_addr">${merchantCurrency.mainAddress}</p>
                                                                </div>

                                                                <button id="address-copy" class="btn btn-danger"
                                                                        style="padding: 0 20px"><loc:message
                                                                        code="refill.copy"/></button>
                                                            </div>
                                                            <%--<loc:message code="merchants.modalOutputAddressTag"/>--%>
                                                            <div class="alert alert-success">
                                                                <div>${merchantCurrency.additionalFieldName}:</div>
                                                                <div id="add-address-to-pay"
                                                                     style="font-size:12px; overflow:auto">
                                                                    <p>${merchantCurrency.address}</p>
                                                                </div>

                                                                <button id="add-address-copy" class="btn btn-danger"
                                                                        style="padding: 0 20px"><loc:message
                                                                        code="refill.copyAdd"
                                                                        arguments="${merchantCurrency.additionalFieldName}"/></button>
                                                            </div>
                                                            <div class="alert alert-warning"><loc:message
                                                                    code="message.additional.address.warning.${currency.getName()}"/></div>
                                                        </c:when>
                                                        <c:otherwise>
                                                            <loc:message
                                                                    code="refill.messageAboutCurrentAddressSimple"/>
                                                    ...
                                                    <div class="timeoutWarning">
                                                        <c:forEach var="warningCode" items="${warningCodeList}">
                                                            <div><strong><loc:message code="${warningCode}"/></strong>
                                                            </div>
                                                        </c:forEach>
                                                    </div>
                                                    <div>
                                                        ...
                                                        <c:choose>
                                                            <c:when test="${merchantCurrency.name == 'LHT'}">
                                                                <img src='https://chart.googleapis.com/chart?chs=100x100&chld=L|2&cht=qr&chl=<c:out value='usdx%3A${merchantCurrency.mainAddress}%3Fcurrency%3D${merchantCurrency.name}%26memo%3D${merchantCurrency.address}%26ro%3Dtrue'/>' />
                                                            </c:when>
                                                            <c:when test="${merchantCurrency.name == 'ARN' || merchantCurrency.name == 'BinanceCoin'}">
                                                                <img src='https://chart.googleapis.com/chart?chs=100x100&chld=L|2&cht=qr&chl=<c:out value="${merchantCurrency.mainAddress}"/>' />
                                                            </c:when>
                                                            <c:when test="${merchantCurrency.name == 'QIWI'}">
                                                                <img src='https://chart.googleapis.com/chart?chs=100x100&chld=L|2&cht=qr&chl=<c:out value="${merchantCurrency.paymentLink}"/>' />
                                                            </c:when>
                                                            <c:otherwise>
                                                    ...
                                                    <c:if test="${merchantCurrency.generateAdditionalRefillAddressAvailable}">
                                                        <button id=address-generate class="btn start-refill"
                                ...
                                <tr>
                                    ...
                                    <td class="center table-button-block" style="text-align: right;">
                                        <form id="inputoutput-center-tableBody__form" class="table-button-block__form">
                                            ...
                                            <@=getButtonsSet(id, sourceType, merchantName, buttons,
                                            "unconfirmed-refills-table")@>
                                        </form>
                                </tr>
                            </script>
                            </tbody>
